...
export default function Dashboard() {
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter(item => {
      const matchesQ = q === "" || item.name.toLowerCase().includes(q) || item.parties.toLowerCase().includes(q);
      ...
      <div className="p-4 max-w-6xl mx-auto">
        ...
        <div className="bg-white p-4 rounded shadow">
          <div className="flex gap-3 mb-4">
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search name or parties" className="flex-1 border rounded p-2" />
            ...
            <tbody>
              {paged.map(c => (
                <tr key={c.id} className="border-b hover:bg-slate-50">
                  <td className="py-3">
                    <Link to={`/contracts/${c.id}`} className="text-indigo-600 hover:underline">{c.name}</Link>
                  </td>
